...
- **每日帳戶淨值 (NAV) 是另一條獨立管線**：由 GitHub Actions 用 IBKR Flex Web Service
  （`fetch_ibkr_nav.py`）每天抓 `nav_history.json`，與此 MCP 持股快照流程**無關**，不需在這裡手動更新。
  規格與設定（token / query id / secrets）見 `IBKR_FLEX_NAV.md`。
